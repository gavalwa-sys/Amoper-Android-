...
You have two options. Option B needs no software installed on your computer
at all — recommended if you just want the `.apk` quickly.

### Option A — Build it yourself in Android Studio
...
### Option B — Build it in the cloud with GitHub Actions (no software to install)

This repo (the folder you unzipped) includes `.github/workflows/build-apk.yml`,
which builds a real, installable debug `.apk` on GitHub's servers.

1. Create a new **GitHub repository** (public or private).
2. Push the *whole unzipped folder* to it — `README.md`, `AMOPER-API/`,
   `AmoperAndroid/`, and `.github/` all need to sit at the repo root:
   ```
   git init
   git add .
   git commit -m "AMOPER Android app"
   git branch -M main
   git remote add origin https://github.com/YOUR_USERNAME/YOUR_REPO.git
   git push -u origin main
   ```
3. Open the **Actions** tab on GitHub. The workflow runs automatically on
   that push (or click "Run workflow" any time after).
4. Wait for the green checkmark (a few minutes), open the run, and
   download the **`amoper-debug-apk`** artifact — a zip containing
   `app-debug.apk`.
5. Get that `.apk` onto your Android phone (email it to yourself, Google
   Drive, USB — any method) and tap it to install, allowing "install from
   unknown sources" when prompted.
6. **Before installing on a real phone**, make sure `API_BASE_URL` in
   `AmoperAndroid/app/build.gradle.kts` points at your real server (not the
   emulator-only default `10.0.2.2` address) — edit, commit, push, and the
   next Actions run will bake in the right URL.
